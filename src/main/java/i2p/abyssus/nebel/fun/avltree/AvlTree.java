 ...
 * <p>Все итераторы продуцируемые деревом, реализуют интерфейс {@link ListIterator}, за тем исключением, что методы {@link ListIterator#add(Object) add()}, {@link ListIterator#nextIndex() nextIndex()}, {@link ListIterator#previousIndex() previousIndex()} и {@link ListIterator#set(Object) set()}, при обращении к ним, выбрасывают исключение {@link UnsupportedOperationException}. Кроме того, если дерево было изменено не средствами конкретного экземпляра итератора, то любые его методы, за исключением перечисленных выше, при попытке обращения, будут выбрасывать исключение {@link ConcurrentModificationException}.</p>
 * <p>Многопоточный доступ к любому из экземпляров класса, как прямой, так и посредством порождённых итераторов, требует наличия внешней синхронизации.</p>
...
// nested classes
	...
	// instance methods
		 ...
		 * <p>Метод возвращает {@code true}, если следующий элемент может быть возвращён итератором, при обращении к методу {@link #next()}. В противном случае, возвращается значение {@code false}.</p>
		 * @return <ul>
		 *     <li>{@code true} - Если следующий элемент может быть возвращён итератором.</li>
...
// instance methods
	...
	/**
	 * Итератор, установленный перед выбранным элементом.
	 * <p>Метод возвращает итератор элементов дерева, установленный перед элементом, с указанным ключом. Если элемент с таким ключом отсутствует в дереве, то итератор будет установлен перед той позицией, на которой мог бы находится указанный элемент. Иными словами, все элементы предшествующие позиции итератора, будут иметь меньшие ключи, а все последующие - большие.</p>
	 * @param key Ключ выбранного элемента.
	 * @return Итератор, установленный перед выбранным элементом.
	 */
	public ListIterator<E> iterator (
		final K key
	) { // method body
		...
		return ((top != null) && (keyComparator.compare(keyExtractor.apply(top.item), key) >= 0))
			? iteratorOfNext(top)
			: iteratorOfPrevious(top);
	} // iterator()
